...
import sbat.logist.ru.controller.ajaxObject.UpdateSeveralRequests;
...
//        return matViewBigSelectRepository.findAll(input);
        ...
        switch (authentication.getRole()) {
            case "MARKET_AGENT":
                return matViewBigSelectRepository.findAll(input, RequestForUser.requestsForMarketAgent(authentication.getUser()));
            case "CLIENT_MANAGER":
                return matViewBigSelectRepository.findAll(input, RequestForUser.requestsForClient(authentication.getUser().getClient()));
            default:
                return matViewBigSelectRepository.findAll(input);
    ...
    public DataTablesOutput<MatViewBigSelect> getMatViewBigSelect() {
        ...
        switch (authentication.getRole()) {
            case "MARKET_AGENT":
                return matViewBigSelectRepository.findAll(new DataTablesInput(), RequestForUser.requestsForMarketAgent(authentication.getUser()));
            case "CLIENT_MANAGER":
                return matViewBigSelectRepository.findAll(new DataTablesInput(), RequestForUser.requestsForClient(authentication.getUser().getClient()));
            default:
                return matViewBigSelectRepository.findAll(new DataTablesInput());
    ...
    @ResponseBody
    @RequestMapping(value = "/commands/updateRequests", method = RequestMethod.POST)
    public int updateSeveralRequestsGet(
            @RequestBody UpdateSeveralRequests updateSeveralRequests
//            @Param("requests") @Valid Request requests
    ) {
        updateSeveralRequests.processEntities(requestReqpository,driverRepository,vehicleRepository,routeListRepository,transportCompanyRepository);
        return 1;
    }
